...
def matrix_t(mD, mL, mU, w):
    md = substract_matrix(mD, multiply_matrix_scalar(mL, w))
    md = inv(md)
    return np.matmul(md, sum_matrix(multiply_matrix_scalar(matrix_d,(1-w)), multiply_matrix_scalar(matrix_u, w)))

def matrix_c(mD, mL, b, w):
    md = substract_matrix(mD, multiply_matrix_scalar(mL, w))
    md = multiply_matrix_scalar(inv(md).tolist(), w)
    return multiply_matrix(md, b)

def multiply_matrix_scalar(m1, s):
    length = len(m1)
    new_matrix = list()
    x = 0
    for element in m1:
...
matrix_t = matrix_t(matrix_d, matrix_l, matrix_u, w)
vector_b = vector_b(b)
matrix_c = matrix_c(matrix_d, matrix_l, vector_b, w)
xant = x0
E = 1000
cont=0
